...
<div class="max-w-4xl mx-auto mb-8 mt-8">
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="text-center mb-6">
            <h3 class="text-2xl font-bold text-gray-800 mb-2">🔍 البحث في قسم عائلتي</h3>
            <p class="text-gray-600">ابحثي عن النصائح العائلية، الأنشطة، والأماكن المناسبة للعائلة</p>
        </div>
        ...
        <form action="{{ route('search') }}" method="GET" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       name="q" 
                       value="{{ request('q') }}"
                       placeholder="ابحثي عن: نصيحة عائلية، نشاط، منطقة خروج..."
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="type" name="type" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 text-right">
                        ...
                        <option value="advices" {{ request('type') == 'advices' ? 'selected' : '' }}>نصائح عائلية</option>
                        <option value="activities" {{ request('type') == 'activities' ? 'selected' : '' }}>أنشطة عائلية</option>
                        <option value="outings" {{ request('type') == 'outings' ? 'selected' : '' }}>مناطق خروج</option>
                        <option value="health" {{ request('type') == 'health' ? 'selected' : '' }}>سجلات صحية</option>
                    </select>
                </div>
