{{-- صفحة أطباء العناية بالبشرة --}}
<x-filament::page>
    <div class="fi-card mb-6 p-4">
        <h3 class="text-lg font-bold mb-4">أطباء العناية بالبشرة</h3>
        <!-- أزرار الإضافة -->
        ...
        <div id="addDoctorForm" style="display:none;">
            <form method="POST" action="{{ route('skin-care-doctors.store') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
        ...
        <div id="addTipForm" style="display:none;">
            <form method="POST" action="{{ route('skin-care-doctors.tips') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="skin_care_doctor_id" class="fi-input">
        ...
        <div id="addVideoForm" style="display:none;">
            <form method="POST" action="{{ route('skin-care-doctors.videos') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="skin_care_doctor_id" class="fi-input">
                    <option value="">بدون ربط بطبيب محدد</option>
                    @foreach($doctors as $d)
            ...
            <table class="fi-ta-table w-full text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('skin-care-doctors.destroy', $doc->id) }}" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <!-- نصائح -->
        <div class="mt-8">
            <h4 class="text-base font-bold mb-2">نصائح العناية بالبشرة</h4>
