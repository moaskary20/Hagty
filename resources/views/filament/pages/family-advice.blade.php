<x-filament-panels::page>
    @php
        $showAdviceModal = request()->old('showAdviceModal', false);
    @endphp
    <div class="min-h-screen bg-gradient-to-br from-pink-50 to-purple-50 p-6" dir="rtl">
        ...
        @endif
        <div class="mb-6">
            <button onclick="document.getElementById('advice-modal').classList.remove('hidden')"
        ...
        <div id="advice-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
            <div class="custom-modal">
                <h2 class="custom-modal-header">إضافة نصيحة جديدة</h2>
                <form method="POST" action="/admin/family-advice/store">
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="content" class="w-full border rounded px-3 py-2" rows="3" required></textarea>
                    </div>
                    <div class="mb-4">
                        <label class="block mb-1 font-medium">اسم الخبير (اختياري)</label>
                        <input type="text" name="expert_name" class="w-full border rounded px-3 py-2">
                    </div>
                    <div class="mb-4">
                        <label class="block mb-1 font-medium">الفئة المستهدفة</label>
                        <input type="text" name="target_audience" class="w-full border rounded px-3 py-2">
                    </div>
                    <button type="submit" class="bg-pink-500 text-white px-4 py-2 rounded w-full font-bold">حفظ النصيحة</button>
                    <button type="button" onclick="document.getElementById('advice-modal').classList.add('hidden')" class="mt-2 bg-gray-300 text-gray-700 px-4 py-2 rounded w-full">إغلاق</button>
                </form>
            </div>
        </div>
    </div>
