{{-- صفحة مشغلو الدي جي والعازفون في قسم فرحي --}}
<x-filament::page>
    <div class="fi-card mb-6 p-4">
        <h3 class="text-lg font-bold mb-4">مشغلو الدي جي والعازفون</h3>
        
        <!-- أزرار الإضافة -->
        <div class="flex flex-wrap gap-2 justify-end mb-4">
            <button type="button" class="fi-btn bg-pink-700 text-lg px-6 py-2" onclick="showDjForm()">
                + إضافة مشغل دي جي
            </button>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="bg-gray-800 rounded-lg p-4">
                    ...
                    @if($video->djPerformer)
                        <p class="text-sm text-gray-400">{{ $video->djPerformer->name }}</p>
                    @endif
                    <p class="text-xs text-gray-500">تخطي بعد {{ $video->skip_after_seconds }} ثانية</p>
                    <form method="POST" action="{{ route('dj-performers.video-ads.destroy', $video->id) }}" style="display:inline;" class="mt-2">
                        @csrf
                        @method('DELETE')
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-gray-800 rounded-lg p-2">
                    ...
                    @if($banner->djPerformer)
                        <p class="text-xs text-gray-400">{{ $banner->djPerformer->name }}</p>
                    @endif
                    ...
                    <div class="flex gap-1 mt-2">
                        ...
                        @endif
                        <form method="POST" action="{{ route('dj-performers.banners.destroy', $banner->id) }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="fi-btn bg-red-600 text-xs py-1 px-2">حذف</button>
                        </form>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="bg-gray-800 rounded-lg p-4 {{ $package->is_popular ? 'border-2 border-yellow-500' : '' }}">
                    ...
                    @if($package->duration_hours)
                        <p class="text-sm text-gray-400 mb-2">المدة: {{ $package->duration_hours }} ساعة</p>
                    @endif
                    @if($package->djPerformer)
                        <p class="text-sm text-gray-400 mb-2">{{ $package->djPerformer->name }}</p>
                    @endif
                    ...
                    @endif
                    <form method="POST" action="{{ route('dj-performers.packages.destroy', $package->id) }}" style="display:inline;">
                        @csrf
                        @method('DELETE')
        ...
        <!-- نموذج إضافة مشغل دي جي -->
        <div id="djForm" style="display:none;">
            <form method="POST" action="{{ route('dj-performers.store') }}" enctype="multipart/form-data" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <h4 class="text-lg font-bold mb-2 text-white">إضافة مشغل دي جي جديد</h4>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <input type="text" name="name" placeholder="الاسم" class="fi-input" required>
                    <input type="text" name="specialty" placeholder="التخصص (دي جي، عازف، فرقة)" class="fi-input">
                </div>
                ...
                <div id="video-inputs">
                    <label class="text-white">روابط فيديوهات المراجع:</label>
                    <div class="flex gap-2 mb-2">
                        <input type="url" name="portfolio_videos[]" placeholder="رابط فيديو" class="fi-input">
                        <button type="button" onclick="addVideoInput()" class="fi-btn bg-green-600 px-3">+</button>
                    </div>
                </div>
                
                <div class="flex gap-2 mt-2">
                    <button type="submit" class="fi-btn bg-pink-600 text-lg py-2 w-full">إضافة</button>
                    <button type="button" onclick="hideDjForm()" class="fi-btn bg-gray-500 w-full">إلغاء</button>
                </div>
            </form>
        </div>
        ...
        <div id="packageForm" style="display:none;">
            <form method="POST" action="{{ route('dj-performers.packages.store') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <h4 class="text-lg font-bold mb-2 text-white">إضافة باقة زفاف</h4>
                
                <select name="dj_performer_id" class="fi-input" required>
                    <option value="">اختر مشغل الدي جي</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                    ...
                    <input type="number" name="duration_hours" placeholder="المدة بالساعات" class="fi-input">
                </div>
        ...
        <div id="bannerForm" style="display:none;">
            <form method="POST" action="{{ route('dj-performers.banners.store') }}" enctype="multipart/form-data" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <h4 class="text-lg font-bold mb-2 text-white">إضافة لافتة عرض</h4>
                
                <select name="dj_performer_id" class="fi-input">
                    <option value="">اختياري: ربط بمشغل دي جي معين</option>
                    @foreach($djPerformers as $dj)
                        <option value="{{ $dj->id }}">{{ $dj->name }}</option>
                    @endforeach
                </select>
        ...
        <div id="videoForm" style="display:none;">
            <form method="POST" action="{{ route('dj-performers.video-ads.store') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
        ...
        <!-- جدول مشغلي الدي جي -->
        <div class="mt-8">
            <h4 class="text-base font-bold mb-2">قائمة مشغلي الدي جي والعازفين</h4>
            <table class="fi-ta-table w-full text-center">
                ...
                <tbody>
                    @forelse($djPerformers as $dj)
                    <tr>
                        <td>
                            <div>
                                <strong>{{ $dj->name }}</strong>
                                @if($dj->email)
                                    <br><a href="mailto:{{ $dj->email }}" class="text-pink-400 text-xs underline">{{ $dj->email }}</a>
                                @endif
                            </div>
                        </td>
                        <td>{{ $dj->specialty ?? '-' }}</td>
                        <td>
                            @if($dj->phone_numbers)
                                @foreach($dj->phone_numbers as $phone)
                        ...
                        <td>
                            @if($dj->portfolio_images && count($dj->portfolio_images) > 0)
                                <div class="flex flex-wrap gap-1 justify-center">
                                    @foreach(array_slice($dj->portfolio_images, 0, 3) as $index => $image)
                                        <img src="{{ asset('storage/' . $image) }}" alt="مرجع" class="w-12 h-12 object-cover rounded cursor-pointer" onclick="showImageModal('{{ asset('storage/' . $image) }}', '{{ $dj->name }} - مرجع {{ $index + 1 }}')">
                                    @endforeach
                                    @if(count($dj->portfolio_images) > 3)
                                        <span class="text-xs bg-gray-600 px-2 py-1 rounded">+{{ count($dj->portfolio_images) - 3 }}</span>
                                    @endif
                                </div>
                            @else
                        ...
                        <td>
                            <div class="flex flex-col gap-1 text-xs">
                                @if($dj->website_url)
                                    <a href="{{ $dj->website_url }}" target="_blank" class="text-blue-400 underline">الموقع</a>
                                @endif
                                @if($dj->instagram_url)
                                    <a href="{{ $dj->instagram_url }}" target="_blank" class="text-pink-400 underline">انستقرام</a>
                                @endif
                                @if($dj->facebook_url)
                                    <a href="{{ $dj->facebook_url }}" target="_blank" class="text-blue-600 underline">فيسبوك</a>
                                @endif
                                @if($dj->portfolio_videos && count($dj->portfolio_videos) > 0)
                                    @foreach(array_slice($dj->portfolio_videos, 0, 2) as $index => $video)
                                        <a href="{{ $video }}" target="_blank" class="text-green-400 underline">فيديو {{ $index + 1 }}</a>
                                    @endforeach
                                @endif
                            </div>
                        </td>
                        <td>
                            @if($dj->starting_price)
                                <span class="text-pink-400 font-bold">{{ number_format($dj->starting_price) }} ريال</span>
                            @else
                        ...
                        <td>
                            <div class="flex flex-col gap-1">
                                <button type="button" onclick="editDj({{ $dj->id }})" class="fi-btn bg-blue-600 text-xs py-1 px-2">تعديل</button>
                                <form method="POST" action="{{ route('dj-performers.destroy', $dj->id) }}" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="fi-btn bg-red-600 text-xs py-1 px-2" onclick="return confirm('هل أنت متأكد من الحذف؟')">حذف</button>
                                </form>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="text-center text-gray-500 py-4">لا توجد بيانات</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <!-- نموذج تعديل مشغل دي جي -->
        <div id="editDjForm" style="display:none;">
            <form id="editForm" method="POST" action="" enctype="multipart/form-data" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                @method('PUT')
                <h4 class="text-lg font-bold mb-2 text-white">تعديل مشغل الدي جي</h4>
                ...
                <button type="button" onclick="addEditVideoInput()" class="fi-btn bg-green-600 px-3 w-32">+ إضافة فيديو</button>
                
                <div class="flex gap-2 mt-2">
                    <button type="submit" class="fi-btn bg-blue-600 text-lg py-2 w-full">تحديث</button>
                    <button type="button" onclick="hideEditDjForm()" class="fi-btn bg-gray-500 w-full">إلغاء</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function showDjForm() {
            hideAllForms();
            document.getElementById('djForm').style.display = 'block';
        }
        function hideDjForm() {
            document.getElementById('djForm').style.display = 'none';
        }
        ...
        function addVideoInput() {
            ...
            const div = document.createElement('div');
            div.className = 'flex gap-2 mb-2';
            div.innerHTML = '<input type="url" name="portfolio_videos[]" placeholder="رابط فيديو" class="fi-input"><button type="button" onclick="removeVideoInput(this)" class="fi-btn bg-red-600 px-3">-</button>';
            container.appendChild(div);
        }
        ...
        // دوال التعديل
        function showEditDjForm() {
            document.getElementById('editDjForm').style.display = 'block';
        }
        function hideEditDjForm() {
            document.getElementById('editDjForm').style.display = 'none';
        }
        
        function editDj(djId) {
            hideAllForms();
            
            const djPerformers = @json($djPerformers);
            const dj = djPerformers.find(d => d.id === djId);
            
            if (dj) {
                document.getElementById('edit_name').value = dj.name || '';
                document.getElementById('edit_specialty').value = dj.specialty || '';
                document.getElementById('edit_email').value = dj.email || '';
                document.getElementById('edit_starting_price').value = dj.starting_price || '';
                document.getElementById('edit_description').value = dj.description || '';
                document.getElementById('edit_website_url').value = dj.website_url || '';
                document.getElementById('edit_instagram_url').value = dj.instagram_url || '';
                document.getElementById('edit_facebook_url').value = dj.facebook_url || '';
                
                document.getElementById('editForm').action = `/dj-performers/${djId}`;
                ...
                if (dj.phone_numbers && dj.phone_numbers.length > 0) {
                    dj.phone_numbers.forEach(phone => {
                ...
                videoContainer.innerHTML = '<label class="text-white">روابط فيديوهات المراجع:</label>';
                if (dj.portfolio_videos && dj.portfolio_videos.length > 0) {
                    dj.portfolio_videos.forEach(video => {
