...
<div class="space-y-6">
    ...
    <div class="fi-card p-6 mb-6">
        ...
        <p class="text-gray-600 text-lg mb-4">كل ما تحتاجينه للاستعداد ليوم الولادة، من النصائح المهمة إلى حقيبة المستشفى وقائمة مستلزمات الطفل.</p>
        
        <!-- أدوات التنقل السريع -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <button onclick="scrollToPreparations()" class="fi-btn bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700">
                📝 النصائح
            </button>
            <button onclick="scrollToHospitalBags()" class="fi-btn bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                🧳 حقيبة المستشفى
            </button>
            <button onclick="scrollToBabyShower()" class="fi-btn bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">
                🍼 مستلزمات الطفل
            </button>
            <button onclick="openAddPreparationModal()" class="fi-btn bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700">
                ➕ إضافة نصيحة
            </button>
        </div>
    </div>
    ...
    <div id="preparationsSection" class="fi-card mb-6">
        <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-xl font-bold">📝 كيفية الاستعداد لولادة الطفل؟</h2>
            <button onclick="openAddPreparationModal()" class="fi-btn bg-purple-600 text-white px-6 py-2 rounded-lg hover:bg-purple-700">
                ➕ إضافة نصيحة
            </button>
        </div>
                    ...
                    <div class="fi-card p-4 hover:shadow-lg transition-all duration-300">
                        ...
                        <div class="p-6">
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                @endif
                                <span class="bg-purple-100 text-purple-800 px-2 py-1 rounded-full text-xs">{{ $preparation->category ?? 'عام' }}</span>
                                @if($preparation->importance_level)
                                    <span class="bg-red-100 text-red-800 px-2 py-1 rounded-full text-xs">أهمية {{ $preparation->importance_level }}/5</span>
                                @endif
                            </div>
                            
                            <h3 class="font-bold text-xl mb-2">{{ $preparation->title }}</h3>
                            <p class="text-gray-600 text-sm mb-4">{{ Str::limit($preparation->description, 120) }}</p>
    ...
    <!-- قسم حقيبة المستشفى -->
    <div id="hospitalBagsSection" class="fi-card mb-6">
        <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-xl font-bold">🧳 حقيبة يوم الولادة للمستشفى</h2>
            <button onclick="openAddBagModal()" class="fi-btn bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">
                ➕ إضافة حقيبة
            </button>
        </div>
    ...
    <div id="babyShowerSection" class="fi-card mb-6">
        <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-xl font-bold">🍼 قائمة مستلزمات الطفل</h2>
            <button onclick="openAddBabyItemModal()" class="fi-btn bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700">
                ➕ إضافة مستلزم
            </button>
        </div>
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="babyItemsContainer">
                @forelse($babyShowerItems as $item)
                    <div class="fi-card p-4 hover:shadow-lg transition-all duration-300 baby-item-card" data-category="{{ $item->category }}">
