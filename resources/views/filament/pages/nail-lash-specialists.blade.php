{{-- صفحة متخصصو الأظافر والرموش --}}
<x-filament::page>
    <div class="fi-card mb-6 p-4">
        <h3 class="text-lg font-bold mb-4">متخصصو الأظافر والرموش</h3>
        <!-- أزرار الإضافة -->
        <div class="flex flex-wrap gap-2 justify-end mb-4">
            <button type="button" class="fi-btn bg-pink-700 text-lg px-6 py-2" onclick="document.getElementById('addSpecialistForm').style.display='block'">
                + إضافة متخصص جديد
            </button>
        ...
        <!-- نموذج إضافة متخصص -->
        <div id="addSpecialistForm" style="display:none;">
            <form method="POST" action="{{ route('nail-lash-specialists.store') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <input type="text" name="name" placeholder="اسم المتخصص" class="fi-input" required>
                ...
                <input type="text" name="center_address" placeholder="عنوان المركز" class="fi-input">
                ...
                <div class="flex gap-2 mt-2">
                    ...
                    <button type="button" onclick="document.getElementById('addSpecialistForm').style.display='none'" class="fi-btn bg-gray-500 w-full">إلغاء</button>
                </div>
            </form>
        </div>
        <!-- نموذج إضافة نصيحة -->
        <div id="addTipForm" style="display:none;">
            <form method="POST" action="{{ route('nail-lash-specialists.tips') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="nail_lash_specialist_id" class="fi-input">
                    <option value="">بدون ربط بمتخصص محدد</option>
        ...
        <div id="addVideoForm" style="display:none;">
            <form method="POST" action="{{ route('nail-lash-specialists.videos') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="nail_lash_specialist_id" class="fi-input">
                    <option value="">بدون ربط بمتخصص محدد</option>
                    @foreach($specialists as $s)
        ...
        <!-- جدول المتخصصين -->
        <div class="mt-8">
            <h4 class="text-base font-bold mb-2">قائمة المتخصصين</h4>
            <table class="fi-ta-table w-full text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $doc->phone }}</td>
                        <td>@if($doc->profile_url)<a href="{{ $doc->profile_url }}" target="_blank" class="text-pink-400 underline">صفحة المتخصص</a>@else - @endif</td>
                        <td>
                            <form method="POST" action="{{ route('nail-lash-specialists.destroy', $doc->id) }}" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <!-- نصائح -->
        <div class="mt-8">
            <h4 class="text-base font-bold mb-2">نصائح الأظافر والرموش</h4>
            <ul class="list-disc pr-6">
                @foreach($tips as $tip)
                    <li>{{ $tip->tip }} @if($tip->specialist) <span class="text-xs text-gray-400">({{ $tip->specialist->name }})</span> @endif</li>
                @endforeach
            </ul>
        </div>
        <!-- فيديوهات -->
            ...
            <div class="flex flex-wrap gap-4">
                @foreach($videos as $video)
                    <div class="bg-gray-800 rounded-lg p-2 flex flex-col items-center" style="width: 220px;">
                        ...
                        @if($video->specialist)
                            <span class="text-xs text-gray-400">({{ $video->specialist->name }})</span>
                        @endif
                    </div>
                @endforeach
            </div>
