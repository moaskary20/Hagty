...
<x-filament::page>
    <div class="fi-card mb-6 p-4">
        <h3 class="text-lg font-bold mb-4">عيادات SPA</h3>
        <!-- أزرار الإضافة -->
        <div class="flex flex-wrap gap-2 justify-end mb-4">
            <button type="button" class="fi-btn bg-pink-700 text-lg px-6 py-2" onclick="document.getElementById('addClinicForm').style.display='block'">
                + إضافة عيادة جديدة
            </button>
        ...
        <!-- نموذج إضافة عيادة -->
        <div id="addClinicForm" style="display:none;">
            <form method="POST" action="{{ route('spa-clinics.store') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <input type="text" name="name" placeholder="اسم العيادة" class="fi-input" required>
                ...
                <div class="flex gap-2 mt-2">
                    ...
                    <button type="button" onclick="document.getElementById('addClinicForm').style.display='none'" class="fi-btn bg-gray-500 w-full">إلغاء</button>
                </div>
            </form>
        </div>
        <!-- نموذج إضافة نصيحة -->
        <div id="addTipForm" style="display:none;">
            <form method="POST" action="{{ route('spa-clinics.tips') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="spa_clinic_id" class="fi-input">
                    <option value="">بدون ربط بعيادة محددة</option>
        ...
        <div id="addVideoForm" style="display:none;">
            <form method="POST" action="{{ route('spa-clinics.videos') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="spa_clinic_id" class="fi-input">
                    <option value="">بدون ربط بعيادة محددة</option>
                    @foreach($clinics as $c)
        ...
        <!-- جدول العيادات -->
        <div class="mt-8">
            <h4 class="text-base font-bold mb-2">قائمة العيادات</h4>
            <table class="fi-ta-table w-full text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $doc->phone }}</td>
                        <td>@if($doc->profile_url)<a href="{{ $doc->profile_url }}" target="_blank" class="text-pink-400 underline">صفحة العيادة</a>@else - @endif</td>
                        <td>
                            <form method="POST" action="{{ route('spa-clinics.destroy', $doc->id) }}" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <!-- نصائح -->
        <div class="mt-8">
            <h4 class="text-base font-bold mb-2">نصائح SPA</h4>
            <ul class="list-disc pr-6">
                @foreach($tips as $tip)
                    <li>{{ $tip->tip }} @if($tip->clinic) <span class="text-xs text-gray-400">({{ $tip->clinic->name }})</span> @endif</li>
                @endforeach
            </ul>
        </div>
        <!-- فيديوهات -->
            ...
            <div class="flex flex-wrap gap-4">
                @foreach($videos as $video)
                    <div class="bg-gray-800 rounded-lg p-2 flex flex-col items-center" style="width: 220px;">
                        ...
                        @if($video->clinic)
                            <span class="text-xs text-gray-400">({{ $video->clinic->name }})</span>
                        @endif
                    </div>
                @endforeach
            </div>
