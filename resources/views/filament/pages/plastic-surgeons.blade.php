{{-- صفحة أطباء جراحة التجميل --}}
<x-filament::page>
    <div class="fi-card mb-6 p-4">
        <h3 class="text-lg font-bold mb-4">أطباء جراحة التجميل</h3>
        <!-- أزرار الإضافة -->
        <div class="flex flex-wrap gap-2 justify-end mb-4">
            <button type="button" class="fi-btn bg-pink-700 text-lg px-6 py-2" onclick="document.getElementById('addSurgeonForm').style.display='block'">
                + إضافة طبيب جديد
            </button>
        ...
        <!-- نموذج إضافة طبيب -->
        <div id="addSurgeonForm" style="display:none;">
            <form method="POST" action="{{ route('plastic-surgeons.store') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                ...
                <div class="flex gap-2 mt-2">
                    ...
                    <button type="button" onclick="document.getElementById('addSurgeonForm').style.display='none'" class="fi-btn bg-gray-500 w-full">إلغاء</button>
                </div>
            </form>
        </div>
        <!-- نموذج إضافة نصيحة -->
        <div id="addTipForm" style="display:none;">
            <form method="POST" action="{{ route('plastic-surgeons.tips') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="plastic_surgeon_id" class="fi-input">
        ...
        <div id="addVideoForm" style="display:none;">
            <form method="POST" action="{{ route('plastic-surgeons.videos') }}" class="flex flex-col gap-2 mb-4 bg-gray-900 p-4 rounded-lg">
                @csrf
                <select name="plastic_surgeon_id" class="fi-input">
                    <option value="">بدون ربط بطبيب محدد</option>
                    @foreach($surgeons as $s)
            ...
            <table class="fi-ta-table w-full text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('plastic-surgeons.destroy', $doc->id) }}" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <!-- نصائح -->
        <div class="mt-8">
            <h4 class="text-base font-bold mb-2">نصائح جراحة التجميل</h4>
            <ul class="list-disc pr-6">
                @foreach($tips as $tip)
                    <li>{{ $tip->tip }} @if($tip->surgeon) <span class="text-xs text-gray-400">({{ $tip->surgeon->name }})</span> @endif</li>
                @endforeach
            </ul>
        </div>
        <!-- فيديوهات -->
            ...
            <div class="flex flex-wrap gap-4">
                @foreach($videos as $video)
                    <div class="bg-gray-800 rounded-lg p-2 flex flex-col items-center" style="width: 220px;">
                        ...
                        @if($video->surgeon)
                            <span class="text-xs text-gray-400">({{ $video->surgeon->name }})</span>
                        @endif
                    </div>
                @endforeach
            </div>
